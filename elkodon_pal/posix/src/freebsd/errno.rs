...
impl Errno {
    pub fn get() -> Errno {
        unsafe { *internal::__error() }.into()
    }

    pub fn correct_errno(wrong_errno_value: Errno, correct_errno_value: Errno) {
        if Errno::get() == wrong_errno_value {
            unsafe { *internal::__error() = correct_errno_value as i32 };
        }
    }
