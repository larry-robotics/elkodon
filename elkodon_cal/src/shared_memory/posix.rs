...
use elkodon_bb_elementary::allocator::DeallocationError;
use elkodon_bb_log::fail;
use elkodon_bb_memory::bump_allocator::BumpAllocator;
use elkodon_bb_posix::shared_memory::{AccessMode, Permission};
use elkodon_bb_posix::system_configuration::SystemInfo;
use elkodon_bb_posix::unix_datagram_socket::CreationMode;
    ...
    crate::shared_memory::SharedMemoryBuilder<Allocator, Memory<Allocator>> for Builder<Allocator>
{
    ...
    fn create(
        ...
        let shm = match elkodon_bb_posix::shared_memory::SharedMemoryBuilder::new(unsafe {
        ...
        {
            Ok(s) => s,
            Err(elkodon_bb_posix::shared_memory::SharedMemoryCreationError::AlreadyExist) => {
                fail!(from self, with SharedMemoryCreateError::AlreadyExists,
                        "{} since a shared memory with that name already exists.", msg);
            }
            Err(
                elkodon_bb_posix::shared_memory::SharedMemoryCreationError::InsufficientPermissions,
            ) => {
    ...
    fn open(self) -> Result<Memory<Allocator>, SharedMemoryOpenError> {
        ...
        {
            Ok(s) => s,
            Err(elkodon_bb_posix::shared_memory::SharedMemoryCreationError::DoesNotExist) => {
                fail!(from self, with SharedMemoryOpenError::DoesNotExist,
                        "{} since a shared memory with that name does not exist.", msg);
            }
            Err(elkodon_bb_posix::shared_memory::SharedMemoryCreationError::SizeDoesNotFit) => {
...
pub struct Memory<Allocator: ShmAllocator> {
    shared_memory: elkodon_bb_posix::shared_memory::SharedMemory,
    name: FileName,
...
impl<Allocator: ShmAllocator + Debug> NamedConceptMgmt for Memory<Allocator> {
    ...
    fn does_exist_cfg(
        ...
        Ok(elkodon_bb_posix::shared_memory::SharedMemory::does_exist(
            &full_name,
        ))
    }
    ...
    fn list_cfg(
        ...
        let entries = elkodon_bb_posix::shared_memory::SharedMemory::list();
