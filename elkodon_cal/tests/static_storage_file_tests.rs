...
fn static_storage_file_path_is_created_when_it_does_not_exist() {
    ...
    let non_existing_path =
        FilePath::from_path_and_file(&TEMP_DIRECTORY, &generate_name()).unwrap();
...
fn static_storage_file_custom_path_and_suffix_list_storage_works() {
    ...
    let config = Configuration::default()
        .suffix(unsafe { FileName::new_unchecked(b".blubbme") })
        .path_hint(
            FilePath::from_path_and_file(&TEMP_DIRECTORY, &FileName::new(b"non_existing").unwrap())
                .unwrap()
                .into(),
        );
    ...
    for _i in 0..NUMBER_OF_STORAGES {
        let storage_name = FilePath::from_path_and_file(&TEMP_DIRECTORY, &generate_name()).unwrap();
