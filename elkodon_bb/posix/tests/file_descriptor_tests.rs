...
use elkodon_pal_posix::posix::{POSIX_SUPPORT_PERMISSIONS, POSIX_SUPPORT_USERS_AND_GROUPS};
...
impl GenericTestBuilder for File {
    fn sut() -> Self {
        let name = FilePath::from_path_and_file(&TEMP_DIRECTORY, &generate_name()).unwrap();
